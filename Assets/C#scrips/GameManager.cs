...
public class GameManager : MonoBehaviour
{
    ...
    private void DoCalculate(int index, OPR opr)
    {
        ...
        float result;  // Initialize result variable
        ...
        switch (opr)
        {
            case OPR.PLUS:
                ...
                break;
            case OPR.MINUS:
                result = leftOperand - rightOperand;  // Fix: Subtract instead of add
                Debug.Log($"{leftOperand} - {rightOperand} : {result}");
                break;
            case OPR.MULTIPLY:
                result = leftOperand * rightOperand;  // Fix: Multiply instead of add
                Debug.Log($"{leftOperand} * {rightOperand} : {result}");
                break;
            case OPR.DIVIDE:
                result = leftOperand / rightOperand;  // Fix: Divide instead of add
                Debug.Log($"{leftOperand} / {rightOperand} : {result}");
                break;
            default:
                result = 0;
